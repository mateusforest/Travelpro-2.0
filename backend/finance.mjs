...
export function filters(input={}){
  ...
  if(!['dueDate','competenceDate'].includes(out.basis)||!['','income','expense','commission','transfer'].includes(out.type)||!['','open','paid','partial','overdue','canceled'].includes(out.status)||!['','manual','legacy','granatum'].includes(out.source)||!Number.isInteger(out.page)||out.page<1||out.page>1000000)fail(422,'Filtro financeiro inválido.');
  return out;
}
...
export function report(rows,catalogs,f){
  ...
  for(const row of found){const e=row.data||row;if(e.canceled||e.type==='transfer')continue;const settled=paid(e),remaining=e.amountCents-settled;summary[e.type==='expense'?'payable':'receivable']+=remaining;summary[e.type==='expense'?'paid':'received']+=settled;if(e.dueDate<f.today)summary.overdue+=remaining;if(e.legacyPaid)summary.legacyUndated++;}
  ...
  for(const row of rows){const e=row.data||row;if(e.canceled||e.type==='transfer'||!matches(e,f,false))continue;for(const p of e.payments)add(p.date,e.type==='expense'?'paid':'received',p.amountCents);add(e.dueDate,e.type==='expense'?'payable':'receivable',e.amountCents-paid(e));}
  const balances=catalogs.filter(c=>c.kind==='account').map(c=>{let balance=c.openingCents;for(const row of rows){const e=row.data||row;if(e.canceled)continue;for(const p of e.payments)if(p.date>=c.openingDate&&p.date<=f.today){if(e.accountId===c.id)balance+=(e.type==='expense'||e.type==='transfer'?-1:1)*p.amountCents;if(e.type==='transfer'&&e.toAccountId===c.id)balance+=p.amountCents;}}return {id:c.id,balanceCents:balance};});
