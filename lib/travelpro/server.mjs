...
import {getUserAccessForUser,ensureAppAccessForUser,canManageWorkspace,resolvePostAuthPath} from '@/lib/auth';
import seed from './initial-state.json';
...
const requestOrigin=request=>{const url=new URL(request.url);return (request.headers.get('x-forwarded-proto')?.split(',')[0].trim()||url.protocol.slice(0,-1))+'://'+(request.headers.get('host')||url.host);};
const clean=(x,max=500)=>typeof x==='string'?x.trim().slice(0,max):'';
const equal=(a,b)=>{const x=Buffer.from(a||''),y=Buffer.from(b||'');return x.length===y.length&&timingSafeEqual(x,y);};
const json=(data,status=200)=>NextResponse.json(data,{status,headers:{'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'}});
...
async function authFlow(path,request,data){
  ...
  if(path==='auth/login'){
    ...
    return json({redirectTo:resolvePostAuthPath(access.access)||'/portal'});
  }
  ...
  if(path==='auth/reset-request'){
    ...
    const {error}=await supabase.auth.resetPasswordForEmail(clean(data.email,254),{redirectTo:origin+'/api/travelpro/auth/callback?flow=recovery'});
...
async function callback(request){
  const url=new URL(request.url);url.host=new URL(requestOrigin(request)).host;const code=url.searchParams.get('code');if(!code)return NextResponse.redirect(new URL('/login?erro=link-invalido',url));
  const supabase=await createSupabaseServerClient();const {data,error}=await supabase.auth.exchangeCodeForSession(code);
  if(error||!data.user)return NextResponse.redirect(new URL('/login?erro=link-expirado',url));
  if(url.searchParams.get('flow')==='recovery'){
    const payload=data.user.id+':'+Date.now();(await cookies()).set('tp-recovery',payload+':'+recoverySignature(payload),{httpOnly:true,sameSite:'lax',secure:url.protocol==='https:',path:'/api/travelpro',maxAge:900});
    return NextResponse.redirect(new URL('/login?recovery=1',url));
  }
  await ensureAppAccessForUser({user:data.user,productType:'operations'});return NextResponse.redirect(new URL('/portal',url));
}

export async function handle(request){
 try{
  const url=new URL(request.url),path=url.pathname.replace(/^\/api\/travelpro\/?/,''),method=request.method;
  ...
  if(path==='auth/reset-confirm'&&method==='POST'){
    ...
    if(!user||parts[0]!==user.id||Date.now()-Number(parts[1])>900000||!equal(parts[2],recoverySignature(payload)))fail(403,'Solicite um novo link de recuperação.');
...
async function whatsappWebhook(request){
 ...
 if(request.method==='GET'){const token=url.searchParams.get('hub.verify_token');if(url.searchParams.get('hub.mode')!=='subscribe'||!token||!connections.some(c=>equal(decrypt(c.secret).verifyToken,token)))fail(403,'Verificação inválida.');return new NextResponse(url.searchParams.get('hub.challenge')||'');}
