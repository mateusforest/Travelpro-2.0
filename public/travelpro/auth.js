(() => {
  ...
  async function finish(form) {
    ...
    try {
      ...
      if(data.message){result.textContent=data.message;return;}location.href=data.redirectTo||'/portal';
  ...
  const token=new URLSearchParams(location.search).get('recovery');
  if(token&&page==='login'){
