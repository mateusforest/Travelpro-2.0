...
import * as validation from '../lib/travelpro/validation.mjs';
import * as providers from '../lib/travelpro/providers.mjs';
const initial=JSON.parse(readFileSync(new URL('../lib/travelpro/initial-state.json',import.meta.url)));
async function fixture({role='owner',workspaceId='agency-a',version=1}={}){
  ...
  class NextResponse extends Response{static json(body,init){return new NextResponse(JSON.stringify(body),{...init,headers:{...init?.headers,'Content-Type':'application/json'}});}static redirect(url){return new NextResponse(null,{status:307,headers:{Location:String(url)}});}}
  const imports={
    'node:crypto':crypto,'next/server':{NextResponse,after:()=>{}},'next/headers':{cookies:async()=>({get:()=>undefined})},
    '@supabase/supabase-js':{createClient:()=>{throw Error('Unexpected external auth');}},
    '@/lib/supabase/server':{createSupabaseServerClient:async()=>({auth:{getUser:async()=>({data:{user}})}}),createSupabaseAdminClient:()=>db},
    '@/lib/auth':{getUserAccessForUser:async()=>access,ensureAppAccessForUser:async()=>({access}),canManageWorkspace:a=>['owner','admin'].includes(a.membershipRole),resolvePostAuthPath:()=>'/portal'},
    './initial-state.json':{default:initial},'./validation.mjs':validation,'./providers.mjs':{...providers,cosReply:async()=>null}
  };
  const module=new vm.SourceTextModule(readFileSync(new URL('../lib/travelpro/server.mjs',import.meta.url),'utf8'));
  ...
  const request=(path,method='GET',body)=>module.namespace.handle(new Request('http://localhost:3000/api/travelpro/'+path,{method,headers:{Host:'localhost:3000',Origin:'http://localhost:3000','Content-Type':'application/json'},body:body===undefined?undefined:JSON.stringify(body)}));
